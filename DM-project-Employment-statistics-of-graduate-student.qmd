...
$$ Where:<br> - $D_{University_j}$: Dummy variables for universities.<br> - $D_{Major_k}$: Dummy variables for majors.<br> - $D_{City_l}$: Dummy variables for cities.<br> - $Duration$: Graduation duration (18 or 30 months).<br>
...
insertion_master_panel |>
  group_by(situation) |>
  summarize(moyenne_taux_d_insertion = mean(`Taux d’insertion`, na.rm = TRUE)) |> knitr::kable(digits = 3, align = "c")
```
...
aggregate_femme |> filter(`Taux de femmes` > average_femme) |>
  ...
  knitr::kable(digits = 3, align = "c", caption = " Specializations with a higher than average proportion of women")
