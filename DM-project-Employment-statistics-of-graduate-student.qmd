...
Some of the observations are on aggreagate data from all degrees of the same discipline, by year. As they are on national data, they are not associated with any specific "académie". We will keep them for analysis purposes but after removing them from the main data set.
...
And to facilitate our future analysis, we also convert some character-formatted data values into numeric format.
...
For this question, we tried to find the "best fields/specializations" from two perspectives. We first conducted a preliminary analysis with the optimal level of employment performance across different fields ('Domaines').

In terms of the insertion rate and stable employment rate, graduates from Arts, Languages, and Literature fields have the lowest employment performance, with both rates below 100%, while all other disciplines achieved a 100% employment rate. Regarding salary levels, graduates in Law, Economics and Management, as well as Science, Technology, and Health, earn the highest pay, reaching €49,100. Graduates from Arts, Languages, and Literature earn the lowest salary at only €34,300.
...
In our survey, there is an average `r paste(average_femme)` % of women, with a standard deviation of `r paste(stdev_femme)` percentage points. We can notice that some specializations have a much higher proportion of women. Keeping only those with a proportion of women higher than the average, we obtain:
...
aggregate_scholarship <- aggregate_data |>
  ...
  group_by(Discipline) |>
  summarise(`Taux de boursiers` = mean(`% de diplômés boursiers`))

#'[Formatting]
aggregate_scholarship |> knitr::kable(caption = "Percentage of graduates with a scholarship per specialization") |>
  row_spec(c(13, 14, 15), background = "yellow")
```

## Conclusion

  We have been able to make significant observations with the various tests performed on both data sets. They are nonetheless insufficient to draw actual conclusions on the situation of graduate students in France. 
The main issue with the main data set is its lack of precision. To ease aggregation, the different graduate degrees were classified in different categories and subcategories (fields and specializations). However, specializations are not necessarily the best indicators for wage and employment outcomes. It does not indicate the type of employment of the graduates. Certain degrees are more likely to lead to civil service, which offers lower wages than the private sector for similar positions, while having more benefits (such as job security).
The fact that all data was collected among individuals at the very beginning of their professional life is another potential critic. People that have just graduated will be in entry-level positions and are unlikely to be liberal professionals. If the same survey were to be performed five to ten years after graduation, the gap in wages between certain specializations would be wider, as some people would have opened their own law firm or medical practice. 
