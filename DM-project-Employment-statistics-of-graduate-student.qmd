...
```{r}
print(insertion_master_panel)
```

In our dataset "insertion_master_panel", many values are missing, due to absence of data ("nd" = non disponible) or unusable data ("ns" =  "non significatif" and "fe" = "faible effectif"). Here, we examine how many missing values we have in each column, and address these missing values carefully before our analysis.
...
There are "summary" observations in our data set : 
...
```{r}
insertion_master_panel |> filter(is.na(Académie))
```
