...
-   First and third quartile regional net monthly salaries. <br>
...
```{r}
#| echo: false 
insertion_master_panel <- insertion_master |> filter(is.na(Remarque)| Remarque == "Université dont les résultats sont plus fragiles (taux de réponse inférieur à 50%)" )
```

In our dataset "insertion_master_panel", we observe many missing values, represented as "ns" or "nd" in some cells. Here, we examine how many missing values we have in each column, and address these missing values carefully before our analysis.
...
```{r}
insertion_master_panel <- insertion_master_panel |>
  mutate(`Taux d’insertion` = ifelse(`Taux d’insertion` == "ns", NA, `Taux d’insertion`),
         `Taux d’insertion` = ifelse(`Taux d’insertion` == "nd", NA, `Taux d’insertion`))
```
...
Which fields/specializations lead to the highest wages, have the highest insertion rate, or have the highest level of stable employment?
...
Number of Établissements
```{r}
insertion_master_panel |> 
  distinct(Établissement) |>
  nrow()
```
