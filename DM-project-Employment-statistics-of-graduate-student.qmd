...
Note: In order to do the regression, we need to convert the variable Établissement into dummy variable. The following code is not the final one, I'm still working on it.
...
aggregate_data |> filter(!is.na(`% emplois extérieurs à la région de l’université`)) |>
  group_by(Domaine) |> summarise(`Taux d'employés en dehors de la région` = mean(`% emplois extérieurs à la région de l’université`))

```


```{r}
insertion_master_panel[, c(26)] <- sapply(insertion_master_panel[, c(26)], function(x){
  if (is.character(x)){
    as.numeric(ifelse(is.na(x), NA, x))
  } else {
    x
...
aggregate_data |>
  filter(!is.na(`% femmes`) & !is.na(`% de diplômés boursiers`)) |>
  group_by(Discipline) |>
  summarise(`Taux de femmes` = mean(`% femmes`), `Taux de boursiers` = mean(`% de diplômés boursiers`)) |> knitr::kable()
```
