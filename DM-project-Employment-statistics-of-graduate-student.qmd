...
Convert the data type of the column "Taux d’insertion" from character to numeric
```{r}
insertion_master_panel$`Taux d’insertion` <- as.numeric(as.character(insertion_master_panel$`Taux d’insertion`))
```
...
```{r}
best_specializations <- insertion_master_panel |>
...
What does Toutes universités et établissements assimilés mean? Can we represent all the obsrvations in the group with missing value in Académie column by a representative Académie (create a new Académie) ? Or just delete all these values ?
