...
summary_result <- summary(regression_uni)

coefficients_table <- as.data.frame(summary_result$coefficients)

coefficients_table$Significance <- symnum(coefficients_table$`Pr(>|t|)`,
                                          cutpoints = c(0, 0.001, 0.01, 0.05, 0.1, 1),
                                          symbols = c("***", "**", "*", ".", " "))
kable(coefficients_table, format = "html", caption = "Regression Coefficients Table",
      col.names = c("Estimate", "Std. Error", "t value", "Pr(>|t|)", "Significance"))
  
```
From the result of our regression, we can easily find that all the variables except Universities play an important role in the annual salary of graduates at the *** significance level. 
...
```{r}
nat_net_avg <- mean(insertion_master_panel$`Salaire net médian des emplois à temps plein`, na.rm = TRUE)
```

The national net monthly average wage is `r wages_profession_gender[10,5]` €, while the net monthly average wage of the surveyed students is `r show(nat_net_avg)` €.
...
Overall, the average insertion rate is `r show(mean_insertion)` . The moment of collection does not seem to be relevant.
...
-   For the majority of "académies", the specialization with the lowest rate of graduates working outside of their region of graduation is education.

-   For the regions of Guadeloupe and Guyane, the lack of observations in our survey makes the results for these zones meaningless as only one specialization is represented.

-   The "académies" with the lowest rates of graduates leaving are Paris, Versailles and Créteil. A possible explanation is that these are the three "académies" of the Île-de-France region, the main economic center of the country.
